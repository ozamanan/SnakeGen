...
curPop0 = np.random.choice(np.arange(-15,15,step=0.01),size=(100,3,4),replace=False)
curPop1 = np.random.choice(np.arange(-15,15,step=0.01),size=(100,4),replace=False)
nextPop = np.zeros((curPop.shape[0], curPop.shape[1]))
fitVec0 = np.zeros((initPop, 2))#1st col is indices, 2nd col is cost
fitVec1 = np.zeros((initPop, 2))
for i in range(numGen):
    #iterate through num generations
    #Create vector of all errors from cost function for each solution
	fitVec0 = np.array([np.array([x, np.sum(NN.costFunction(NN.X, NN.y, curPop[x].T))]) for x in range(initPop)])
	fitVec1 = np.array([np.array([x, np.sum(NN.costFunction(NN.X, NN.y, curPop[x].T))]) for x in range(initPop)])
