...
dirs = 0
score = 0
applepos = (random.randint(0,550), random.randint(0, 550))
pygame.init()
...
while True:
	...
	i = len(xs)-1
	while i >= 2:
		if collide(xs[0], xs[i], ys[0], ys[i], 10, 10, 10, 10):die(s, score)
		i-= 1
	if collide(xs[0], applepos[0], ys[0], applepos[1], 10, 10, 10, 10):score+=1;xs.append(700);ys.append(700);applepos=(random.randint(0,550),random.randint(0,550))
	if xs[0] < 0 or xs[0] > 550 or ys[0] < 0 or ys[0] > 550: die(s, score)
	i = len(xs)-1
	while i >= 1:
